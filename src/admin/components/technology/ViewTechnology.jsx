...
import { deleteTechnology, getTechnologies } from "../../../services/technology";
...
const ViewTechnology = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const formattedRows = data?.map((item, index) => ({
          id: item._id,
          No: index + 1,
          title: item.images.map((image) => image.title).join(", ") || "N/A",
          image: item.images[0].url || "N/A",
  ...
  const handleEdit = () => {
    handleMenuClose();
    navigate(`/development/dashboard/edit-technology/${selectedId}`);
  };
  ...
  const handleDelete = async () => {
    ...
    const result = await Swal.fire({
      title: "Are you sure?",
      text: "Do you really want to delete this technology? This action cannot be undone.",
      icon: "warning",
